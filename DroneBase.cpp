...
const std::vector<Model> SceneToLoad = {
	{"Terrain.obj", "desert.jpg", {0,0,0}, 1, Flat},
	{"Terrain.obj", "Colors.png", {0,0,0}, 1, Flat},
	{"drone.obj", "Colors2.png", {0,0,0}, 1, Flat},
	{"Terrain.obj", "Colors.png", {0,0,0}, 1, Wire},
...
//For setting a Skybox
struct SkyBoxModel {
	const char *ObjFile;
	const char *TextureFile[6];
};

//const SkyBoxModel SkyBoxToLoad = {"SkyBoxCube.obj", {"sky/sky1.jpg", "sky/sky2.jpg", "sky/sky3.jpg", "sky/sky4.jpg", "sky/sky5.jpg", "sky/sky6.jpg"}};
const SkyBoxModel SkyBoxToLoad = { "SkyBoxCube.obj", {"sky/clouds1_west.bmp", "sky/clouds1_east.bmp", "sky/clouds1_up.bmp" , "sky/clouds1_down.bmp" , "sky/clouds1_north.bmp", "sky/clouds1_south.bmp"} };
struct SingleText {
	int usedLines;
	const char *l[4];
	int start;
...
std::vector<SingleText> SceneText = {
	{1, {"Press Space to take off", "", "", ""}, 0, 0},
	{1, {"Navigate in the area!", "", "", ""}, 0, 0},
};
...
// The uniform buffer object used for the project
...
// TheGlobal  uniform buffer object used for the project
...
private:
	...
	// Drone Pos
	glm::vec3 DronePos = glm::vec3(0,0,0);
	...
	static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
		auto app = reinterpret_cast<DroneSimulator*>
    ...
    void initVulkan() {
		...
		createDescriptorSetLayouts(); //L21
		createPipelines();			  //
		createCommandPool();
...
//throw std::runtime_error("Now We Stop Here!");			
	...
	//Functionn to load each model with the appropriate texture
...
//std::cout << k << " " << j << " " << i << " " << vb << " " << ib << " " << c << "\n";
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		const float ROT_SPEED1 = glm::radians(90.0f);
		...
		glm::vec3 oldDronePos = DronePos;

	


		if (DronePos != glm::vec3(0, DronePos.y, 0)) {

			

			if (glfwGetKey(window, GLFW_KEY_L)) {
				lookRoll = 0;
				lookPitch += deltaT * ROT_SPEED1 * 500;
			...
			if (glfwGetKey(window, GLFW_KEY_LEFT)) {
				lookRoll += deltaT * ROT_SPEED * 50;
			}
			if (glfwGetKey(window, GLFW_KEY_RIGHT)) {
				lookRoll -= deltaT * ROT_SPEED * 50;
			}
			if (glfwGetKey(window, GLFW_KEY_UP)) {
				lookYaw += deltaT * ROT_SPEED * 50;
			}
			...
			if (glfwGetKey(window, GLFW_KEY_A)) {
				DronePos -= MOVE_SPEED * glm::vec3(1.0f, 0.0f, 0.0f) * deltaT *speed ;
			}
			...
			if (glfwGetKey(window, GLFW_KEY_W)) {
				DronePos -= MOVE_SPEED * glm::vec3(0.0f, 0.0f, 1.0f) * deltaT * speed;
			}
			if (glfwGetKey(window, GLFW_KEY_Z)) {
				DronePos += MOVE_SPEED * glm::vec3(0.0f, 0.0f, 1.0f) * deltaT * speed;
			}
			if (glfwGetKey(window, GLFW_KEY_U)) {
				DronePos.y += MOVE_SPEED * deltaT;
			}
		...
		if (glfwGetKey(window, GLFW_KEY_SPACE)) {
			...
			if (DronePos == glm::vec3(0.0f, DronePos.y, 0.0f)) {
					if (DronePos.y < 1.8) {
						lookPitch += deltaT * ROT_SPEED1 * 500;
						DronePos.y += MOVE_SPEED * deltaT;
...
//std::cout << round(lookYaw * 180.f / 3.1416f) << "\t" << round(lookPitch * 180.f / 3.1416f) << "\t" <<  round(lookRoll * 180.f / 3.1416f) << "\n";
		...
		static glm::vec3 FollowerPos = DronePos;


		CamMat = glm::lookAt(glm::vec3((DronePos.x + 1.f)*0.8, (DronePos.y + 2.f)*0.6, (DronePos.z + 3.f)*0.8),
			glm::vec3(DronePos.x, DronePos.y, DronePos.z),
...
//std::cout << "Making invisible object " << j << "\n";
			...
			if(j==2) {
				glm::mat4 RobWM = glm::rotate(glm::translate(glm::mat4(1), DronePos),
									  lookYaw, glm::vec3(0,0,1));
				
				glm::quat qe = glm::quat(glm::vec3(glm::radians(lookYaw), 0, 0))*
					glm::quat(glm::vec3(0, glm::radians(lookPitch), 0))
					* glm::quat(glm::vec3(0, 0, glm::radians(lookRoll)));

				ubo.mMat = glm::translate(glm::mat4(1.0f), DronePos)*glm::mat4(qe);
						
				FollowerTargetPos = RobWM * glm::translate(glm::mat4(1), FollowerDeltaTarget) *
									glm::rotate(glm::mat4(1), lookPitch, glm::vec3(0,0,1)) *
